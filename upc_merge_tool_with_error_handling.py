...
st.set_page_config(page_title="🔄 UPC Merge Tool (Improved Error Handling)", layout="wide")
st.title("🔄 UPC Merge Tool (with Smart Upload Handling)")

st.markdown("""
✅ NEW: Improved support for file errors and user guidance

- Manual column mapping if headers are unclear
- Skips empty or malformed sheets with a warning
- Validates required columns before proceeding
- Detects category hierarchy columns with `>` (even if unnamed)
""")
...
if upc_file and partner_file:
    ...
    if not valid_sheets:
        st.error("❌ No usable sheets found. Make sure your file has at least one sheet with valid data.")
    else:
        ...
        cat_col = None
        for col in columns:
            if upc_df[col].astype(str).str.contains('>').any():
                cat_col = col
                break
        ...
        st.write(f"📦 Category Column: `{cat_col if cat_col else 'Not detected'}`")

        if not desc_col or not upc_col:
            st.error("❌ You must define both a description and barcode column.")
        else:
            if st.button("🚀 Merge & Download"):
                ...
                st.download_button(
                    ...
                    data=output,
                    file_name="merged_error_handling_output.xlsx",
